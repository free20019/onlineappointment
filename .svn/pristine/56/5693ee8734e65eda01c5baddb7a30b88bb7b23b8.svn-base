...
public class AsyncService {
    ...
    //用户预约
    @Async
    public void dealAppointRecord(Appoint appoint, ReturnAppointResult appointResult){
        SimpleDateFormat sdf= new SimpleDateFormat("yyyy年MM月dd日");
        Company company = appointResult.getCompany();
        ManageConfigure configure = appointResult.getManageConfigure();
        ...
        //1,更新预约记录
        int appoint_insert = 0;
        try {
            //将预约记录存入数据库
            appoint_insert = appointMapper.update(appoint, new QueryWrapper<Appoint>()
